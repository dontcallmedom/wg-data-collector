...
async function listTRSpecs(gid) {
  ...
  for (let spec of trSpecs) {
    ...
    spec.repo = (groupRepos[spec.tr] && groupRepos[spec.tr].repo) ? `${groupRepos[spec.tr].repo.owner}/${groupRepos[spec.tr].repo.name}` : '';
...
async function listSpecs(gid) {
  ...
  const nonTRSpecs =  await listNonTRSpecs(gid, trSpecs);
