import {fetchJSON, implementations} from "./utils.mjs";
import fetchIdlImplData from "idl-implementation-tracker";

export default async function getIdlImplData(shortname) {
  let implementationData;
  try {
    const data = await fetchIdlImplData(shortname);
    if (data.error) {
      throw (data.error);
    }
